...
export default {
  ...
  methods: {
    getData() {
      this.loading = true
      this.$store
        .dispatch('systems/getAll', { limit: this.perPage, offset: this.getOffset, search: this.searchTerm })
        .then(({ data }) => {
          this.data = data
    ...
    getNewUrl() {
      return `${process.env.VUE_APP_ORACULO_ADMIN_URL}/catalog/system/`
    },
  },
}
</script>
            ...
            <div v-else class="table-responsive">
              <b-table :items="items" :fields="fields" hover class="table-custom">
                <template #cell(name)="data">
                  <router-link :to="`/systems/${data.item.id}`">{{ data.value }}</router-link>
                </template>
