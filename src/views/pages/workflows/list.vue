...
export default {
  ...
  data: () => ({
    ...
    fields: [
      ...
      { key: 'system', label: 'System', sortable: true, tdClass: 'align-middle' },
  ...
  methods: {
    getData() {
      this.loading = true
      this.$store
        .dispatch('workflows/getAll', { limit: this.perPage, offset: this.getOffset, search: this.searchTerm })
        .then(({ data }) => {
          this.data = data
    ...
    getNewUrl() {
      return `${process.env.VUE_APP_ORACULO_ADMIN_URL}/catalog/workflow/`
    },
  },
}
</script>
            ...
            <div v-else class="table-responsive">
              <b-table :items="items" :fields="fields" hover class="table-custom">
                <template #cell(name)="data">
                  <router-link :to="`/workflows/${data.item.id}`">{{ data.value }}</router-link>
                </template>
                <template #cell(description)="data">
                  {{ removeSpaces(removeHtmlTags(data.value)) }}
                </template>
                <template #cell(system)="data">
                  <span v-if="data.item.system">{{ data.item.system.name }}</span>
                </template>
