  ...
  <div id="sidebar-menu">
    ...
    <ul id="side-menu" class="metismenu list-unstyled">
      <template v-for="(item, index) in menuItems">
        <li v-if="item.isTitle" :key="index" class="menu-title">
          {{ $t(item.label) }}
        </li>
        <li v-if="!item.isTitle && !item.isLayout" :key="index">
          ...
          >
            <feather v-if="item.icon" :type="`${item.icon}`" class="icon nav-icon"></feather>
            <span>{{ $t(item.label) }}</span>
            <span v-if="item.badge" :class="`badge rounded-pill bg-${item.badge.variant} float-end`">{{
              $t(item.badge.text)
            }}</span>
          </a>
          <router-link v-if="!hasItems(item)" :to="item.link" class="side-nav-link-ref">
            <feather v-if="item.icon" :type="`${item.icon}`" class="icon nav-icon"></feather>
            <span>{{ $t(item.label) }}</span>
            <span v-if="item.badge" :class="`badge rounded-pill ${item.badge.variant} float-end`">{{
              $t(item.badge.text)
            }}</span>
          </router-link>
          ...
          <ul v-if="hasItems(item)" class="sub-menu" aria-expanded="false">
            <li v-for="(subitem, i) of item.subItems" :key="i">
              <router-link
                v-if="!hasItems(subitem) && subitem.link !== undefined"
                :to="subitem.link"
